...
/* !
 * \file broadcast_sch_nlast_transpose_ub_broadcast_base.h
 * \brief
 */
...
namespace Ops{
namespace Base {
constexpr uint64_t NUM_FOUR = 4;
constexpr uint64_t NUM_THREE = 3;
constexpr uint64_t NUM_TWO = 2;

template <class BrcDag, bool UseNddma, int64_t R = -1>
class BroadcastNlastTransposeBaseSch
{
public:
    __aicore__ inline explicit BroadcastNlastTransposeBaseSch(
...
protected:
     ...
     */
    template <size_t N>
    __aicore__ inline void GetUbBroadcastShapeInfo(const int64_t (&oriShape)[N], uint32_t (&ubFormershape)[N])
    {
    ...
    /* *
     * 获取shape对应的StridesWithPad信息
     * @tparam N shape的长度
     * @param oriDims 要计算对应的shape
     * @param stridesWithPad 要计算的StridesWithPad
     * @return void
     */
    template <size_t N>
    __aicore__ inline void GetStridesWithPadThroughDims(const int64_t (&oriDims)[N], uint32_t (&stridesWithPad)[N])
    {
        int64_t tmp_strides[BROADCAST_NON_CONTIGIOUS_MAX_DIMS_NUM] = {
            1, 1, 1, 1}; /* 计算 viewshape 连续后对应的 stride*/
        int64_t base = 1;
        for (int64_t j = tilingData_->shapeLen - 1; j >= 0; j--) {
            tmp_strides[j] = base;
            base *= oriDims[j];
        }

        // 逐个拷贝生成inputStridesWithPad
        for (int64_t i = tilingData_->shapeLen - 1; i >= 0; i--) {
            if (i > tilingData_->shapeLen - NUM_TWO) {
                stridesWithPad[i] = tmp_strides[i];
            } else if (i == tilingData_->shapeLen - NUM_TWO) {
                int64_t oriStride = tmp_strides[i];
                int64_t oriStrideAlign = (oriStride + tilingData_->minDtypeBlockAlignSize - 1) /
                                         tilingData_->minDtypeBlockAlignSize * tilingData_->minDtypeBlockAlignSize;
                stridesWithPad[i] = oriStrideAlign;
            } else {
                stridesWithPad[i] = stridesWithPad[i + 1] * oriDims[i + 1];
            }
        }
    }
    ...
    __aicore__ inline void GetVecBrcInfo()
    {
        ...
        GetUbBroadcastShapeInfo(tilingData_->inputVecBrcDims[index], vecBrcFormerShape_[index]);
...
protected:
    ...
    uint32_t vecBrcFormerShape_[VEC_BRC_SIZE][4] = {{1}};
    uint32_t vBrcFormer_[VEC_BRC_SIZE] = {1};
    uint32_t vBrcTail_[VEC_BRC_SIZE] = {1};
    uint32_t dstUbFormerShape_[4] = {1};
    uint32_t dstUbFormerShapeLastPad_[4] = {1};
...
#endif // BROADCAST_SCH_NLAST_TRANSPOSE_UB_BROADCAST_BASE_H_