...
def do_pkg_softlink(target_conf=None,
            delivery_dir='',
            release_dir='',
            package_name=None):
    '''
...
def safe_copy(src, dst):
    """
    复制文件：如果目标文件存在且无写权限，先加写权限，复制后恢复原权限
    """
    original_mode = None
    if os.path.exists(dst):
        original_mode = os.stat(dst).st_mode
        os.chmod(dst, original_mode | 0o200)
...
def main(pkg_name='', xml_file='', main_args=None):
    ...
    if main_args.independent_pkg:
        ...
        safe_copy(src_file_path, os.path.join(dst_file_path, "filelist.csv"))
