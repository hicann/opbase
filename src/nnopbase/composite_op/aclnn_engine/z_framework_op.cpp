...
static inline aclnnStatus CalcTensorNBytes(const aclTensor* tensor, uint64_t& nBytes)
{
    ...
    if (tensorTypeSize > kDataTypeSizeBitOffset) {
        OP_CHECK(
            ...
            OP_LOGE(ACLNN_ERR_INNER, "Tensor size is too large, calc tensor nbytes overflow."), return ACLNN_ERR_INNER);

        nBytes = (nBytes + BYTE_BITS_MINUS_ONE) >> LOG_2_BYTE_BITS;
        return ACLNN_SUCCESS;
    } else {
        OP_CHECK(
            !ge::MulOverflow((tensorTypeSize), tensor->GetStorageShape().GetShapeSize(), nBytes),
            OP_LOGE(ACLNN_ERR_INNER, "Tensor size is too large, calc tensor nbytes overflow."), return ACLNN_ERR_INNER);
        return ACLNN_SUCCESS;
    }
}
...
public:
    CopyToNpuKernelLauncher(
        uint32_t opType, op::CoreType coreType, op::internal::ProfilingInfoId& profilingId,
    ...
    aclnnStatus Launch() override
    {
        ...
        auto ret = aclrtMemcpyAsync(
            dst_->GetData(), dstNByptes, src_->GetData(), srcNByptes, ACL_MEMCPY_HOST_TO_BUF_TO_DEVICE,
            executor_->GetStream());
        OP_CHECK(
            ret == ACL_SUCCESS,
...
public:
    ...
    aclnnStatus Launch() override
    {
        ...
        OP_CHECK(
            ...
            return ACLNN_ERR_INNER);
        auto ret = aclrtMemcpyAsync(
            dst_->GetData(), dstNByptes, src_->GetData(), srcNByptes, ACL_MEMCPY_DEVICE_TO_DEVICE,
            executor_->GetStream());
        OP_CHECK(
            ret == ACL_SUCCESS,
...
const aclTensor* CopyToNpu(const aclTensor* src, aclOpExecutor* executor)
{
    ...
    OP_CHECK(
        src->GetPlacement() == op::TensorPlacement::kOnHost,
        OP_LOGE(ACLNN_ERR_INNER, "Get input param's placement:%d when expect kOnHost.", src->GetPlacement()),
        return nullptr);
    auto dst = executor->AllocTensor(
        src->GetStorageShape(), src->GetOriginalShape(), src->GetDataType(), src->GetStorageFormat(),
        src->GetOriginalFormat());

    op::internal::ProfilingInfoId profilingInfoId;

    auto* launcher = new CopyToNpuKernelLauncher{CopyToNpuOpTypeId(), op::NO_CALC, profilingInfoId, executor, src, dst};
    OpArg srcArg;
    ...
    auto ret =
        executor->AddToKernelLauncherListCopyTask(CopyToNpuOpTypeId(), launcher, srcArgList, dstArgList, emptyArgList);
    OP_CHECK(
        ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER, "Build input output relation for workspace calc failed"),
        return nullptr);
    return dst;
}
...
aclnnStatus CopyNpuToNpu(const aclTensor* src, const aclTensor* dst, aclOpExecutor* executor)
{
    ...
    OP_CHECK(
        src->GetPlacement() == op::TensorPlacement::kOnDeviceHbm,
        OP_LOGE(ACLNN_ERR_INNER, "Get input param's placement:%d when expect kOnDeviceHbm.", src->GetPlacement()),
        return ACLNN_ERR_INNER);
    OP_CHECK(
        dst->GetPlacement() == op::TensorPlacement::kOnDeviceHbm,
    ...
    OpArgList dstArgList(&dstArg, 1);
    OpArgList emptyArgList;
    auto ret = executor->AddToKernelLauncherListCopyTask(
        CopyNpuToNpuOpTypeId(), launcher, srcArgList, dstArgList, emptyArgList);
    OP_CHECK(
        ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER, "Build input output relation for workspace calc failed"),
        return ACLNN_ERR_INNER);
    return ACLNN_SUCCESS;
