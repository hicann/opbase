...
public:
    ...
    {
        OP_LOGI("Destroy one-shot aicpu launcher, opType=%u, task_alive=%d, args_alive=%d.",
            opType_, task_ != nullptr, args_ != nullptr);
        if (args_) {
    ...
    aclnnStatus Launch() override
    {
        ...
        auto ret = task_->SetIoTensors(const_cast<aclOpExecutor *>(executor_), args_);
        if (ret != OK) {
            return ret;
        }
        ret = task_->Run(const_cast<aclOpExecutor *>(executor_), executor_->GetStream());
        return ret;
    }
                                     ...
                                     const FVector<std::string> &attrNames, op::OpArgContext *args)
{
    ...
    if (overflowTask != nullptr) {
        ...
        launcher = new op::internal::AiCpuKernelLauncher{
