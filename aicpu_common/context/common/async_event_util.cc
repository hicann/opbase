...
void AsyncEventUtil::InitEventUtil()
{
    ...
    reg_event_cb_with_times_func_ =
        reinterpret_cast<RegEventCbWithTimesFunc>(dlsym(sharder_, kRegEventCbWithTimesFunc));
...
AsyncEventUtil::AsyncEventUtil()
{
    ...
    if (sharder_ == nullptr) {
        KERNEL_LOG_ERROR("Device sharder dlopen so [%s] failed, error[%s]", kSharderPath, dlerror());
...
bool AsyncEventUtil::RegEventCb(
...
bool AsyncEventUtil::RegOpEventCb(
    const uint32_t event_id, const uint32_t sub_event_id, const std::function<void(void*)>& cb) const
{
    if (reg_op_event_cb_func_ != nullptr) {
