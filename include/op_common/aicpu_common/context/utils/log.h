...
#define KERNEL_LOG_DEBUG(fmt, ...)                                                            \
    do {                                                                                      \
        if ((&CheckLogLevel != nullptr) && (&DlogRecord != nullptr)) {                        \
            dlog_debug(AICPU, "[%s][tid:%ld]" fmt, __func__, aicpu::GetTid(), ##__VA_ARGS__); \
        }                                                                                     \
    } while (0)
...
#define KERNEL_LOG_INFO(fmt, ...)                                                            \
    do {                                                                                     \
        if ((&CheckLogLevel != nullptr) && (&DlogRecord != nullptr)) {                       \
            dlog_info(AICPU, "[%s][tid:%ld]" fmt, __func__, aicpu::GetTid(), ##__VA_ARGS__); \
        }                                                                                    \
    } while (0)
...
#define KERNEL_LOG_WARN(fmt, ...)                                                            \
    do {                                                                                     \
        if ((&CheckLogLevel != nullptr) && (&DlogRecord != nullptr)) {                       \
            dlog_warn(AICPU, "[%s][tid:%ld]" fmt, __func__, aicpu::GetTid(), ##__VA_ARGS__); \
        }                                                                                    \
    } while (0)
...
#define KERNEL_LOG_ERROR(fmt, ...)                                                            \
    do {                                                                                      \
        if (&DlogRecord != nullptr) {                                                         \
            dlog_error(AICPU, "[%s][tid:%ld]" fmt, __func__, aicpu::GetTid(), ##__VA_ARGS__); \
        }                                                                                     \
    } while (0)
