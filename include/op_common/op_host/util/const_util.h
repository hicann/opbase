...
static bool inline GetValueToInteger(const gert::Tensor* constTensor, T2& value)
{
    ...
    OP_CHECK_IF(
        constTensor->GetSize() <= 0, OP_LOGE("GetValueToInteger", "const tensor GetSize is not larger than zero."),
        return false);
...
{
    static_assert(
        std::is_same<typename std::remove_const<ContextType>::type, gert::InferShapeContext>::value ||
    ...
    switch (dtype) {
        case ge::DT_UINT64: {
            OP_CHECK_IF(
                !(GetValueToInteger<uint64_t, T>(constTensor, value)),
                OP_LOGE("GetConstInt", "Get value uint64_t failed."), return false);
            break;
        }
        case ge::DT_INT64: {
            OP_CHECK_IF(
                !(GetValueToInteger<int64_t, T>(constTensor, value)),
                OP_LOGE("GetConstInt", "Get value int64_t failed."), return false);
            break;
        }
        case ge::DT_UINT32: {
            OP_CHECK_IF(
                !(GetValueToInteger<uint32_t, T>(constTensor, value)),
                OP_LOGE("GetConstInt", "Get value uint32_t failed."), return false);
            break;
        }
        case ge::DT_INT32: {
            OP_CHECK_IF(
                !(GetValueToInteger<int32_t, T>(constTensor, value)),
                OP_LOGE("GetConstInt", "Get value int32_t failed."), return false);
            break;
        }
        default: {
            OP_LOGW(
                context->GetNodeName(), "GetConstInt only support [int32, int64, uint64, uint32]. but is %s",
...
bool GetConstIntToShape(const ContextType* context, int64_t inputIdx, gert::Shape& shape)
{
    ...
    const gert::Tensor* constTensor = context->GetInputTensor(inputIdx);
    OP_CHECK_IF(
        constTensor == nullptr, OP_LOGE("GetConstIntToShape", "the constTensor is nullptr, return false"),
        return false);
    ...
    switch (constDtype) {
        case ge::DT_INT32: {
            OP_CHECK_IF(
                !(GetValueToShape<int32_t>(constTensor, shape)),
                OP_LOGE("GetConstIntToShape", "Get value to shape int32_t failed."), return false);
            break;
        }
        case ge::DT_INT64: {
            OP_CHECK_IF(
                !(GetValueToShape<int64_t>(constTensor, shape)),
                OP_LOGE("GetConstIntToShape", "Get value to shape int64_t failed."), return false);
            break;
        }
        case ge::DT_UINT64: {
            OP_CHECK_IF(
                !(GetValueToShape<uint64_t>(constTensor, shape)),
                OP_LOGE("GetConstIntToShape", "Get value to shape uint64_t failed."), return false);
            break;
        }
        case ge::DT_UINT32: {
            OP_CHECK_IF(
                !(GetValueToShape<uint32_t>(constTensor, shape)),
                OP_LOGE("GetConstIntToShape", "Get value to shape uint32_t failed."), return false);
            break;
        }
        default: {
            OP_LOGW(
