...
OPBASE_API int32_t CheckLogLevelInner(int32_t moduleId, int32_t level);
...
#define OP_LOGE_LIBOPAPI_REPORT(opName, fmt, ...)                                                             \
    do {                                                                                                      \
        if (Ops::Base::CheckLogLevelInner(static_cast<int>(OP_MODULE_ID), DLOG_ERROR) == 1) {                 \
            Ops::Base::DlogRecordInner(static_cast<int>(OP_MODULE_ID), DLOG_ERROR,                            \
                                       "[%s:%d][%s]"                                                          \
...
#define OpLogSub(moduleId, level, opInfo, fmt, ...)                                                           \
    do {                                                                                                      \
        if (Ops::Base::CheckLogLevelInner(static_cast<int>(moduleId), level) == 1) {                          \
            Ops::Base::DlogRecordInner(static_cast<int>(moduleId), level,                                     \
                                       "[%s:%d][%s]"                                                          \
                                       "[%s][%" PRIu64 "] OpName:[%s] " fmt,                                  \
                                       __FILE__, __LINE__, OP_SUBMOD_NAME, __FUNCTION__, Ops::Base::GetTid(), \
