...
const size_t primeList[] = {
    ...
    725980837ul,  785430967ul,  849749479ul,  919334987ul,  994618837ul,  1076067617ul, 1164186217ul, 1259520799ul,
    1362662261ul, 1474249943ul, 1594975441ul, 1725587117ul, 1866894511ul, 2019773507ul, 2185171673ul, 2364114217ul,
    2557710269ul, 2767159799ul, 2993761039ul, 3238918481ul, 3504151727ul, 3791104843ul, 4101556399ul, 4294967291ul};
...
struct ListHead {
    ...
    inline bool Empty() { return next_ == this; }
...
public:
    ...
    inline ListHead* Head() { return lruList_.next_; }

    inline ListHead* Tail() { return lruList_.prev_; }

    inline ListHead* Sentinel() { return &lruList_; }
    ...
    void Del(ListHead& entry) { entry.Del(); }
...
public:
    ...
    OpCacheContainerIterator(const OpCacheContainerIterator<KeyType, ValueType>& iter) : ptr_(iter.ptr_) {}
...
public:
    ...
    OpCacheContainer(const hasher& hash = hasher(), const key_equal& equal = key_equal()) : hasher_(hash), equal_(equal)
    {}

    ~OpCacheContainer() { clear(); }
    ...
    inline iterator begin() { return empty() ? end() : iterator(static_cast<pointer>(lru_.Head()), lru_.Sentinel()); }

    inline iterator end() { return iterator(nullptr, lru_.Sentinel()); }
    ...
    inline iterator rend() { return iterator(nullptr, lru_.Sentinel()); }

    inline size_t size() const { return entryNum_; }

    inline bool empty() const { return entryNum_ == 0; }

    inline size_t bucket(const KeyType& key) { return hasher_(key) % bucketNum_; }

    inline size_t bucket_count() const { return bucketNum_; }
...
private:
    inline HlistHead& GetBucket(const key_type& key) { return buckets_[bucket(key)]; }
