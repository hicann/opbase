...
/**
 * @brief FP4 E1M2 floating-point format (extension format)
 *
 ...
 *         +---+--+---+
 *
 * Note: This is an extension format, NOT part of OCP MX v1.0 specification.
 * OCP MX v1.0 only defines E2M1 as the FP4 format (MXFP4).
 *
 * This format follows the same general principles as OCP MX formats:
 * - Bias: 1
...
struct Float4E1M2 {
    ...
    // Constants for E1M2 format (OCP MX MXFP4)
    ...
    constexpr Float4E1M2(uint8_t bits, [[maybe_unused]] FromBitsTag fromBits) : value(bits & 0x0F)
    {
    }
    ...
    template<typename T>
    Float4E1M2 &operator=(T other)
    {
        value = FloatToFloat4E1M2(static_cast<float>(other)).value;
        return *this;
    }
    ...
    Float4E1M2 &operator=(const double &dVal)
    {
        value = FloatToFloat4E1M2(static_cast<float>(dVal)).value;
        return *this;
    }
    ...
    {
        return fp16_t(Float4E1M2ToFloat(*this));
    }
    ...
    static constexpr Float4E1M2 Epsilon()
    {
        ...
        return Float4E1M2(0x01, FromBits());  // 0 0 01 = denorm 0.25
    }

    static constexpr Float4E1M2 Highest()
    {
        // exp=1, man=3 -> 2^0 * 1.75 = 1.75 (0 1 11 = 0x7)
        return Float4E1M2(0x07, FromBits());
    }

    static constexpr Float4E1M2 Lowest()
    {
        // OCP MX E1M2 has no NaN, all bit patterns are valid
        // 1 1 11 = sign=1, exp=1, man=3 -> -2^0 * 1.75 = -1.75
        return Float4E1M2(0x0F, FromBits());  // 1 1 11 = -1.75
    }

    static constexpr Float4E1M2 MinPositiveNormal()
    {
        // 2^0 = 1.0 (0 1 00)
        return Float4E1M2(0x04, FromBits());
    }
    ...
    bool IsNaN() const
    {
        // OCP MX E1M2 has no NaN representation
...
private:
    ...
    static float Float4E1M2ToFloat(Float4E1M2 fp4)
    {
        ...
        // OCP MX E1M2 has no NaN, all bit patterns are valid numbers
        ...
        if (exp == 0) {
            // Denormalized number
            // OCP MX E1M2: value = 2^(1-bias) * (man/4) = 2^0 * (man/4) = man/4
            if (man != 0) {
                ...
                fp32Man = (tempMan & MAN_MASK) << (FP32_MAN_LEN - MAN_BITS);
            } else {
                fp32Exp = 0;
            }
        } else {
            // Normal number: value = 2^(1-1) * (1 + man/4) = (1 + man/4)
            fp32Exp = 1 - EXP_BIAS + FP32_EXP_BIAS;  // 127
            fp32Man |= (1 << FP32_MAN_LEN);  // Add implicit 1
        }
    ...
    {
        // OCP MX E1M2 has no NaN, clamp NaN to positive max value (1.75)
        ...
        if (exp == 0 && man == 0) {
            // Zero
            return Float4E1M2(static_cast<uint8_t>(sign << 3), FromBits());
        }

        if (std::isinf(f)) {
            // E1M2 has no infinity, clamp to max value
            // 0x07 = +1.75, 0x0F = -1.75
            return Float4E1M2(static_cast<uint8_t>((sign << 3) | 0x07), FromBits());
        }
        ...
        if (fp4Exp <= 0) {
            ...
            // OCP MX E1M2 has no NaN, clamp to max value (preserving sign)
            // 0x07 = +1.75, 0x0F = -1.75
            return Float4E1M2(static_cast<uint8_t>((sign << 3) | 0x07), FromBits());
        }
...
public:
    ...
    static constexpr bool has_quiet_NaN = false;  // OCP MX E1M2 has no NaN
    ...
    {
        // OCP MX E1M2 has no NaN, return zero as fallback
