...
inline Format GetPrimaryFormat(Format format) { return static_cast<Format>(static_cast<uint32_t>(format) & 0xffU); }
...
inline int64_t GetC0Format(int32_t format)
{
    return static_cast<int64_t>(
        1 << (static_cast<int32_t>((static_cast<uint32_t>(format) & 0xf000000U) >> BIT_THREE_BYTES) - 1));
}
