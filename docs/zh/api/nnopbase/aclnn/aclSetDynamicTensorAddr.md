...
通过[aclSetAclOpExecutorRepeatable](aclSetAclOpExecutorRepeatable.md)开启aclOpExecutor可复用后，若**输入或输出Device内存地址变更**，需要刷新**对应aclTensorList**中记录的Device内存地址。
...
| irIndex | 输入 | 待刷新的aclTensorList在算子原型定义中的索引，从0开始计数。 |
