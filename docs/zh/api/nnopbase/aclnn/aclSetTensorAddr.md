...
通过[aclSetAclOpExecutorRepeatable](aclSetAclOpExecutorRepeatable.md)开启aclOpExecutor可复用后，若**输入或输出Device内存地址变更**，需要刷新**对应aclTensor**中记录的Device内存地址。
...
| index | 输入 | 待刷新的aclTensor索引，取值范围是[0, tensor的总数-1]。 |
