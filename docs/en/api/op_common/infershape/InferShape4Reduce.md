...
Infers an output tensor's shape. It is the `InferShape` for `Reduce` operators in graph mode.
...
```cpp
// Apply inferShape to the IsFinite operator. The procedure follows the same process as Reduce operators, allowing direct reuse.
