...
After aclOpExecutor reuse is enabled by the [aclSetAclOpExecutorRepeatable](aclSetAclOpExecutorRepeatable.md) call, if the input or output device memory address changes, the device memory address recorded in the aclTensor needs to be updated.
...
| index | Input| Index of aclTensor to be updated. Value range: [0, total number of tensors – 1].|
...
void *addr;
aclSetTensorAddr(executor, 0, tensor1, addr); // Update the device address of the first aclTensor in the input tensor list.
