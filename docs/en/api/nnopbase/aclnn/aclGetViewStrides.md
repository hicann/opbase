...
Obtains the ViewStrides of the aclTensor, that is, the stride corresponding to the ViewShape. The aclTensor is created by the [aclCreateTensor](aclCreateTensor.md) call.
...
| stridesValue | Output| Returned ViewStrides value.|
| stridesNum | Output| Number of stride values of ViewStrides.|
...
- If error code 161001 is returned, the `tensor`, `stridesValue`, or `stridesNum` parameter is a null pointer.

## Restrictions

The memory for the `stridesValue` parameter is internally allocated by this API. After use, you need to manually release it.
